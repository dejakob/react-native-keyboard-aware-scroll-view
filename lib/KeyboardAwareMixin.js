...
const KeyboardAwareMixin = {
  mixins: [TimerMixin],
  propTypes: {
    enableAutoAutomaticScroll: PropTypes && PropTypes.bool,
    keyboardOpeningTime: PropTypes && PropTypes.number,
    extraHeight: PropTypes && PropTypes.number,
    extraScrollHeight: PropTypes && PropTypes.number,
    enableResetScrollToCoords: PropTypes && PropTypes.bool,
  },
  ...
  getScrollResponder() {
    const ref = this.refs._rnkasv_keyboardView;
    ...
    if (ref) {
      ...
      // In case of Animated ScrollView
      if (
        ref._component &&
        typeof ref._component.getScrollResponder === 'function'
      ) {
        return ref._component.getScrollResponder();
      }
    }
